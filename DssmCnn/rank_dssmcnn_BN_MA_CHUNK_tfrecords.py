...
class Ranking_DSSMCNN_tfrecords(object):
    ...
    def __init__(self,
        ...
        self.input_centre = input_centre
        ...
        for i, filter_size in enumerate(filter_sizes):
            ...
            with tf.name_scope("conv-maxpool-left-%s" % filter_size):
                ...
                for i in range(C):
                    if i == C - 1:
                        h_i_List.append(
                            tf.slice(h, [0, i * int(int(b) / C), 0, 0],
                                     [-1, int(int(b) - i * int(int(b) / C)), int(c), int(d)]))
                        break
                    h_i_List.append(tf.slice(h, [0, i * int(int(b) / C), 0, 0], [-1, int(int(b) / C), int(c), int(d)]))

                h_outputs = []
                for h0 in h_i_List:
                    if h0 == h_i_List[-1]:
                        pooled = tf.nn.max_pool(h0,
                                                ksize=[1, b - i * int(int(b) / C), 1, 1],
                                                strides=[1, 1, 1, 1],
                                                padding='VALID',
                                                name="pool")
                        ...
                        break
                    pooled = tf.nn.max_pool(h0,
                                            ksize=[1, int(int(b) / C), 1, 1],
                                            strides=[1, 1, 1, 1],
                                            padding='VALID',
                                            name="pool")  # pooled: [batch_size, 1, 1, out_channels]
                    h_outputs.append(pooled)
                print("pooled", pooled)
                h_stack_left = tf.stack(values=h_outputs, axis=4)
                (a, b, c, d, e) = h_stack_left.shape
                h_reshape_left = tf.reshape(h_stack_left, [-1, 1, 1, int(d * e)])
            ...
            with tf.name_scope("conv-maxpool-right-%s" % filter_size):
                ...
                for i in range(C):
                    if i == C - 1:
                        h_i_List.append(
                            tf.slice(h, [0, i * int(int(b) / C), 0, 0],
                                     [-1, int(b - i * int(int(b) / C)), int(c), int(d)]))
                        break
                    h_i_List.append(tf.slice(h, [0, i * int(int(b) / C), 0, 0], [-1, int(int(b) / C), int(c), int(d)]))
        ...
        print("pooled_outputs_left", pooled_outputs_left)
        ...
        print("self.h_pool_left", self.h_pool_left)
