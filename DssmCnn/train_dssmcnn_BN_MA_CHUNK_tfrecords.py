...
def processTFrecords(wordVocab, savePath, max_len=25, fileNumber=2):
    ...
    for i in range(fileNumber):
        ...
        for line in open(openFileName):
            ...
            query3 = line[3]
            query3 = pad_sentence(query3.split(" "), sequence_length=max_len)
            query3 = wordVocab.to_index_sequenceList(query3)
            if len(query3) != max_len: continue
            ...
            example = tf.train.Example(
                features=tf.train.Features(feature={
                    ...
                    'query3': tf.train.Feature(
                        int64_list=tf.train.Int64List(value=query3))
                }))
...
def main(_):
    ...
    totalLines = countLines(train_path)
